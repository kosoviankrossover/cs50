...
function translators () {
  ...
  {
    alert("Please provide a language from which to translate.");
  }
  // check that a "to" language is chosen
  else if (to_lang == "0")
  {
    alert("Please provide a language to which to translate.");
  }
...
function translate_back () {
  ...
  m_where =$("#m_div");
  // first empty the div to avoid duplicates
  m_where.empty();
...
function GoogleTranslate (text, from, to) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (g_method == 1)
      {
        // append the translation of the translate text to the Google div in bold
...
// create global variable to store current token for MicrosoftTranslate to work
var global_token = '';
 
// grab a token or renew it when needed
function pageLoaded() {
  // obtain access token (usable for 10 minutes)
  getToken();
